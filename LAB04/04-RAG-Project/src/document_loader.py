...
# document_loader.py
# Load data/car_q_a.txt and convert it into a list of Q&A records.
...
def load_qa_file(file_path):
    ...
    for line_no, raw in enumerate(lines, start=1):
        ...
        if line.startswith("หมวด") or line.startswith("[หมวด"):
            category = line.strip("[]").replace("หมวด:", "").strip()
        elif re.match(r"^\d+\.\s*Q:", line) or line.startswith("Q:"):
            question = re.sub(r"^\d+\.\s*Q:", "", line).strip() if re.match(r"^\d+\.\s*Q:", line) else line[2:].strip()
